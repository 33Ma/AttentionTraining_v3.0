...
class ModeSelectDialog(QDialog):
    ...
    def _show_mode_info(self):
        ...
        track_group.setText("""
            ...
            <p><b>🎯 目标：</b>在目标点消失前快速点击命中，保持手眼协调与持续专注。</p>
            <p><b>🎮 玩法：</b></p>
            <ul>
                <li>每帧同时出现多个红色目标点，每个点在 0.8 秒后消失</li>
                <li>鼠标光标扫过目标点即可命中，无需点击；点击空白处会计入失误</li>
                <li>命中率、平均响应时间、追踪路径效率加权得到游戏得分</li>
            </ul>
            <p><b>💡 技巧：</b></p>
            <ul>
                <li>预判目标点位置，提前移动鼠标</li>
                <li>保持手眼协调，快速准确点击</li>
                <li>鼠标移动路径越直，路径效率得分越高</li>
            </ul>
